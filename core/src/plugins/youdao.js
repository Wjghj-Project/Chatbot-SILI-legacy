...
template.set('youdao', {
  error_unkown: '未知错误',
  info_lang_not_support: '注意：不支持的语言{0}将被替换为自动识别。',
  info_failed: '翻译失败：{0}',
})
...
const API_ENDPOINT = require('../secret/api').youdao
...
function getErrorDesc(code) {
  ...
  return list[code] || template('youdao.error_unknown')
}

async function makeQuery(q, from, to) {
  if (!from) from = 'auto'
  if (!to) to = 'auto'
  const { data } = await axios({
    url: API_ENDPOINT,
    method: 'post',
    data: stringify({ q, from, to }),
  })
  return data
}
  ...
  ctx
    ...
    .action(async ({ session, options }, text) => {
      ...
      if (!from || !to) {
        ...
        session.send(template('youdao.info_lang_not_support', not.join('和')))
      }
      ...
      try {
        ...
        let phonetic = ''
        if (isWord) {
          if (data?.basic?.phonetic) phonetic = `[${data.basic.phonetic}]` || ''
