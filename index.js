...
const wordsFilter = require('@dragon-fish/words-filter')
...
koishi.start().then(() => {
  ...
  discord.on('message', msg => {
    ...
    content = content.replace(
      /<:.+?:(.+?)>/gi,
      '[CQ:image,file=https://discord-emoji.vercel.app/api/get/?id=$1]'
    )
    if (msg.attachments) {
      try {
  ...
  koishi
    ...
    .receiver.on('message', meta => {
      ...
      if (/\[\[.+\]\]/g.test(meta.message)) {
        ...
        axios
          .get(
            'https://community.fandom.com/api.php?action=query&format=json&prop=info&inprop=url&iwurl=1&titles=' +
              encodeURI(pageName)
          )
          .then(
            res => {
  ...
  koishi
    ...
    .option('-l, --lang <lang>', '搜索的语言，例如zh，预设en', {
      default: 'en'
    })
  ...
  koishi.receiver.on('message/normal', meta => {
    var { message, messageId, groupId } = meta
    var filter = new wordsFilter({
      list: ['过滤器测试']
    })
    if (filter.isBad(message)) {
      koishi.bot.deleteMessage(messageId)
      koishi.sender.sendGroupMsg(groupId, '触发过滤器')
    }
  })
  ...
  koishi
    ...
    .action(({ meta }) => {
      if (meta.$user.authority >= 4) {
        ...
        koishi.database.mysql
          .query(
            "UPDATE `user` SET `authority` = '4' WHERE `user`.`id` = " +
              qqNumber.user.xiaoyujun +
              ';'
          )
          .then(res => {
            meta.$send('done')
