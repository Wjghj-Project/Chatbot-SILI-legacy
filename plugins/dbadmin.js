function apply (koishi, pluginOpt) {
  ...
  koishi
    ...
    .example('dbadmin.set --user=onebot:123456 nickname foo 将标识符为onebot:123456的用户数据中的nickname字段值修改为foo')
    ...
    .action(async ({ session, options: opt }, key, val) => {
      ...
      if (opt.user && opt.user !== true) {
        let user = opt.user.split(':')
        if (user.length < 2) return '未指定用户所在平台，您指的可能是：onebot:' + opt.user

        let data = {}
        data[key] = val

        console.log('dbadmin.set - user', user, data)
        let ret = await session.database.setUser(user[0], user[1], data)
        console.log(ret)

        session.send(`用户信息已修改：\n用户：${opt.user}\n键：${key}\n值：${val}`)
        return
      }
  ...
  koishi
    ...
    .example('dbadmin.get --user=onebot:123456 nickname 获取标识符为onebot:123456的用户数据中的nickname字段的值')
    .option('user', '-u <uid>', { type: 'string' })
    .option('channel', '-c <channel>', { type: 'string' })
    .action(async ({ session, options: opt }, key) => {
      ...
      if (opt.user && opt.user !== true) {
        ...
        let ret = await session.database.getUser(user[0], user[1], 
key.split(','))
        console.log(ret)

        session.send(`用户 ${opt.user} 的信息：\n${JSON.stringify(ret, null, 2)}`)
        return
      }
