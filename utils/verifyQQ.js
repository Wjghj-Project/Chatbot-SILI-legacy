...
function verifyQQ(options, next) {
  var msg = '',
    status = false
  if (!options.user) {
    // meta.$send('未指定用户名')
  ...
  axios.get('https://community.fandom.com/zh/api.php', {
    ...
    if (data.parse && data.parse.revid) {
      ...
      if (verifyNumber !== encodeNumber) {
        ...
        axios.get('https://community.fandom.com/zh/api.php', {
          ...
          if (lastEditor === userName) {
            status = true
            msg = '[CQ:at,qq=' + qqNumber + '] [' + userName + '] 验证通过！'
          } else {
            status = false
            msg = '[CQ:at,qq=' + qqNumber + '] [' + userName + '] 验证失败，最后编辑者为' + lastEditor + '！'
          }
          next && next({msg, status})
        })
      }
      ...
      status = false
      msg = '[CQ:at,qq=' + qqNumber + '] [' + userName + '] 验证失败，' + encodeURI('https://community.fandom.com/zh/wiki/User:' + userName + '/verify-qq') + ' 不存在！'
      next && next({msg, status})
    }
  })
}
